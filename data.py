...
def recommendFilm():
    avgFilmMarks = []
    for i in range(1, data.shape[1] + 1):
        avgFilmMarks.append(data[" Movie " + str(i)].where(data[" Movie " + str(i)] != markNotSet).mean())
    homePart = []
    for i in range(1, context_place.shape[1] + 1):
    ...
    for i in range(1, context_day.shape[1] + 1):
        ...
        for key, value in all.items():
            if (value.strip() != str(markNotSet)):
                watched += 1
                if (value.strip() == 'Sun' or value.strip() == 'Sat'):
                    at_weekends += 1
